...
---
## 想想自己使用的场景
通常我们是先从缓存中读取,如果有,返回;如果没有,则从数据库中读取，写入缓存,设置缓存时间,并返回。   
代码如下：
``` java
...
```
写到这里是不是发现还会有一个问题？就是如果 userName 在其他地方还会更新呢？ 所以一定还有一段：
``` java
